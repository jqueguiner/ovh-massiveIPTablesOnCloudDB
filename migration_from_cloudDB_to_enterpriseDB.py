...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migration from OVH CloudDB to Enterprise Cloud DB"
    )
    ...
    parser.add_argument(
        "--destination_database_force_overwrite",
        default=True,
        type=str2bool,
    ...
    enterprise_create_security_group(
        args.destination_service, args.destination_security_group
    )

    transfer_ip_whitelisting(
    ...
    destination_user = enterprise_create_user_password(
        args.destination_service, args.destination_user_password
    )
...
def transfer_ip_whitelisting(
    ...
    if include_local_ip:
        ...
        print(
            "[WARNING] Not adding local IP will potentially make the rest of the migration failed !"
        )

    ip_restriction_enterpriseDB = get_ip_restrictions(
        "enterpriseDB", destination_service, destination_security_group
    )
...
def api_setup(ovh_config_file):
    ...
    if not "application_key" in config[config["default"]["endpoint"]]:
        ...
        replace_in_file(
            ovh_config_file,
            r"(;application_key=.*)",
            r"application_key=" + application_key,
        )
        application_secret = raw_input("Please enter your Application Secret: ")
        replace_in_file(
            ovh_config_file,
            r"(;application_secret=.*)",
            r"application_secret=" + application_secret,
        )
    ...
    if not "consumer_key" in config[config["default"]["endpoint"]]:
        ...
        replace_in_file(
            "ovh.conf",
            r"(;consumer_key=.*)",
            r"consumer_key=" + validation["consumerKey"],
        )


def enterprise_get_security_groups(service_name):
    security_groups = client.get(
    ...
    for security_group in security_groups:
        security_group_details = client.get(
...
def enterprise_create_security_group(service_name, security_group_name=""):
    if enterpise_security_group_exists(service_name, security_group_name):
        ...
        print(
            "[INFO] Setting up a new security group (%s) for service %s"
            % (security_group_name, service_name)
        )
...
def enterprise_create_user_password(service_name, password):
    ...
    client.post(
        "/cloudDB/enterprise/cluster/" + service_name + "/user", password=password
    )
...
def get_ip_restrictions(db_type, service_name, security_group_name=""):
    ...
    if db_type == "cloudDB":
        ...
        existing_ip_tables = client.get(
            "/hosting/privateDatabase/" + service_name + "/whitelist",
            service=True,
            sftp=True,
        )
        ...
        if security_group_name == "":
            ...
            security_group_name = raw_input(
                "Please tell me which Security Group Name you want to get ip tables from : "
            )

        security_group_id = enterprise_get_security_group_by_name(
            service_name, security_group_name
        )
...
def set_ip_restrictions(
    ...
    if db_type == "cloudDB":
        ...
        security_group_id = enterprise_get_security_group_by_name(
            service_name, security_group_to_update
        )
...
def dump_db(db_type, service_name, db_name, move_to_readonly):
    ...
    if db_type == "cloudDB":
        ...
        try:
            ...
            print(
                "[INFO] Dump already in progress for database %s in service %s"
        ...
        while ready == False:
            print(
                "[INFO] Waiting for dump for database %s in service %s"
                % (db_name, service_name)
            )
        ...
        print(
            "[INFO] Dump ready for database %s in service %s with dump_id = %s"
            % (db_name, service_name, str(dump_id))
        )

        print(
            "[INFO] Downloading dump for database %s in service %s with id %s "
            % (db_name, service_name, str(dump_id))
        )
...
def get_rw_endpoint(service_name):
    ...
    for endpoint in endpoints:
        ep = client.get(
            "/cloudDB/enterprise/cluster/" + service_name + "/endpoint/" + endpoint
        )
...
def restore_db(dump_file, service_name, username, password, db_name, force_overwrite):
    ...
    if db_name not in all_dbs:
        ...
        if force_overwrite:
            ...
            print(
                "[FATAL ERROR] Exiting : As Database exists we can't import the existing database."
            )
            sys.exit(0)

    cmd = (
        "gzip -d --stdout %s |  PGPASSWORD=%s psql --host %s --username %s --port %d %s"
        % (dump_file, password, fqdn, username, port, db_name)
    )
